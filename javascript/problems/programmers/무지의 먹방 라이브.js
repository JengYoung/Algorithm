class MinHeap {
  ...
  heappop() {
    ...
    while (
      ...
      Math.min(this.heap[leftIndex][1], this.heap[rightIndex][1]) <
        this.heap[nowIndex][1]
    ) {
      const minIndex =
...
const solution = (food_times, k) => {
  ...
  while (total + (now - last) * remainFoodCount <= k) {
    // [1000000, 2000000, 3000000] [1, 1000000] 1000000 * 3
